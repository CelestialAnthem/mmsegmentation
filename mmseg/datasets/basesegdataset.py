...
@DATASETS.register_module()
class BaseHaomoDataset(BaseDataset):
    """Custom dataset for semantic segmentation. An example of file structure
    is as followed.
    ...
    def load_data_list(self) -> List[dict]:
        ...
        if not osp.isdir(self.ann_file) and self.ann_file:
            ...
            _suffix_len = len(self.img_suffix)
            if self.anno_json_path is not None:
                with open('/' + self.anno_json_path, 'r') as fp:
                    json_map = json.load(fp)
                for item in json_map:
                    data_info = {}
                    data_info['img_path'] = item['img_path']
                    data_info['seg_map_path'] = item['seg_map_path']
                    data_info['label_map'] = self.label_map
                    data_info['reduce_zero_label'] = self.reduce_zero_label
                    data_info['seg_fields'] = []
                    data_list.append(data_info)
